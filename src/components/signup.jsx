...
const Signup = () => {
    ...
    const dataSubmit = async (e) => {
        ...
        if (name && password && email != 0) {
            try {
                ...
                localStorage.setItem("usersignup", JSON.stringify(result.result));
    ...
    return (
        <>
            <h1 className="mb-4 text-2xl font-extrabold leading-none tracking-tight text-center mt-5 mb-5 pb-5 text-gray-900 md:text-4xl lg:text-4xl dark:text-white">Register Here</h1>
            <form className="max-w-md mx-auto">
                <div className="relative z-0 w-full mb-5 group">
                    <input type="text" value={name} autoComplete="off" onChange={e => setName(e.target.value)} id="floating_first_name" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
                ...
                <button type="submit" onClick={dataSubmit} className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Submit</button>
            </form>
